...
comments within ${js} breaks it.

"// Don't focus if clicking inside room-actions" is followed by just string collored markup. the first node is colored red though. meaning it's breaking the coloring.

FIXED: Added comment parsing to scanTemplateExpr function to handle // and /* */ comments properly within ${} expressions.

Test case with various comment types:
```javascript
html`
    <div
        onclick=${() => {
